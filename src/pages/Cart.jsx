...
function Cart() {
  ...
  const { items, totalCount, totalPrice } = useSelector(({ cart }) => cart); //из редакса беру корзину

  const addedPizzas = Object.keys(items).map((key) => {
    return items[key].items[0]; //взял первый элемент
  });
  ...
  return (
    <div className="content">
      <div className="container container--cart">
        {totalCount ? (
          <div className="cart">
            {/* если totalCount есть...если пиццы есть,то показываю все,либо */}
